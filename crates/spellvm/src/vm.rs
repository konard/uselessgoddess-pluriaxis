...
use crate::outcome::{
    BASE_COUNT, BASE_DURATION_MS, BASE_RANGE, DURATION_STEP_MS, Form, MAX_BOUNCE, MAX_COUNT,
    MAX_POWER, RANGE_STEP, SpellComponent, SpellOutcome,
};
...
impl Vm {
    ...
    fn fuse(&mut self) {
        ...
        let charge = (a.charge as u16 + b.charge as u16).min(MAX_POWER as u16) as u8;
    ...
    fn finalize_form(&mut self, form: Form) -> Result<(), VmError> {
        let ingredient = self
            .stack
            .pop()
            .unwrap_or_else(|| Ingredient::new(Element::Aether));
        let p = core::mem::take(&mut self.pending);

        let power = (ingredient.charge as u16 + p.amplify as u16).clamp(1, MAX_POWER as u16) as u8;
        let count = (BASE_COUNT as u16 + p.split as u16).clamp(1, MAX_COUNT as u16) as u8;
